<!-- Modal Edit Stock -->
<div x-show="isVisible == 'stock-edit-modal'" x-transition:enter="transition ease-out duration-200"
    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
    x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
    x-transition:leave-end="opacity-0" @click.self="isVisible = 'stock-table'"
    x-on:keydown.escape.window="isVisible = 'stock-table'"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div
            ...
            <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 pb-4 md:pb-5">
                ...
                <button type="button" @click="isVisible = 'stock-table'"
            ...
            <!-- Modal body -->
            <form @submit.prevent="editDataStock">
                <div class="py-4 md:py-6 px-2">
                    <div class="grid grid-cols-2 gap-6">

                        <!-- Stock ID (read-only) -->
                        <div class="col-span-2">
                            <label for="edit_stock_id"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-100">Stock ID</label>
                            <input type="text" :value="editStock.id" id="edit_stock_id" disabled
                                class="bg-gray-100 dark:bg-gray-600 border border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 text-sm rounded-lg block w-full px-3 py-2.5 cursor-not-allowed">
                        </div>
                        ...
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <input type="number" x-model="editStock.quantity" id="edit_stock_quantity"
                            ...
                            <template x-if="errors.quantity && errors.quantity.length > 0">
                                <p class="mt-2.5 text-sm text-red-500" x-text="errors.quantity"></p>
                            </template>
                        </div>

                        <!-- Status -->
                        <div class="col-span-2 md:col-span-1">
                            <label for="edit_stock_status"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-100">Status</label>
                            <select x-model="editStock.status" id="edit_stock_status"
                            ...
                            <template x-if="errors.status && errors.status.length > 0">
                                <p class="mt-2.5 text-sm text-red-500" x-text="errors.status"></p>
                            </template>
                        </div>
                ...
                <div
                    ...
                    <button type="submit" :disabled="isLoading"
                        ...
                        <span x-text="isLoading ? 'Loading...' : 'Update stock'"></span>
                    </button>
                </div>
            </form>
        </div>
